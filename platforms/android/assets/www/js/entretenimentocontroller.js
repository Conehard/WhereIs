  ...
  var onSuccess = function(position) {
    ...
    $http.get('https://maps.google.com/maps/api/geocode/json?latlng='+$scope.latlong+'&sensor=false&key=AIzaSyD03iJeEEgdhw1O0oTK5qqtd2Iy6uAzJAE').success(function (data) {
		...
		$http.get('http://egendnetwork.com/whereis/comercios.php?cidade='+$scope.newmapa).success(function (datacomercio) {
		    ...
		    if($scope.comercio == ""){
				$scope.erro = "Ainda não existem locais de entretenimento cadastrados nesta cidade.";
		    };
		    $scope.filterSub = { };
		    ...
		    $scope.erro = "Não foi possível obter os locais de entretenimento, tenha certeza de estar conectado a internet para utilizar o aplicativo. Caso esteja e ainda esteja vendo este erro, aguarde alguns segundos e tente novamente.";
		}).finally(function () {
			$scope.loading = false;
	    });
